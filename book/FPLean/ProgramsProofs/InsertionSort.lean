...
⊢ (match compare arr[j'] arr[j' + 1] with
    ...
    arr.size
```
When faced with a goal that includes {kw}`if` or {kw}`match`, the `split` tactic (not to be confused with the `split` function used in the definition of merge sort) replaces the goal with one new goal for each path of control flow:
