...
To get started with a project that uses Lake, use the command {command lake "first-lake"}`lake new greeting` in a directory that does not already contain a file or directory called `greeting`.
...
Running {command lake "first-lake/greeting"}`./.lake/build/bin/greeting` results in `{{#command_out {lake} {./.lake/build/bin/greeting} }}`.
Instead of running the binary directly, the command `lake exe` can be used to build the binary if necessary and then run it.
Running {command lake "first-lake/greeting"}`lake exe greeting` also results in `{{#command_out {lake} {lake exe greeting} }}`.
