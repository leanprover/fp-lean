...
To prove `plusR_zero_left` with the induction tactic, begin by writing its signature (using {kw}`theorem`, because this really is a proof).
...
Just as `pure ()` can be used in a {kw}`do` block to indicate "do nothing", the tactic language has a statement `skip` that also does nothing.
