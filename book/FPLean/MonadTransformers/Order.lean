...
{anchorTerm M1eval}`M1 α` unfolds to {anchorTerm M1eval}`LetterCounts → Except Err (α × LetterCounts)`, and {anchorTerm M2eval}`(
M2 α
) = (
LetterCounts → Except Err α × LetterCounts
)` unfolds to {anchorTerm M2eval}`LetterCounts → Except Err α × LetterCounts`.
