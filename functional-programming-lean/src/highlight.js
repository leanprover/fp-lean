...
// A version of the Lean language above with special highlighting enabled for tactics.
// This is because Lean 4 can't really be highlighted with regexps, because a token that's a tactic name may be a variable elsewhere.
hljs.registerLanguage("leantac", function(hljs) {
  var LEAN_KEYWORDS = {
    $pattern: /#?\w+/,
    keyword:
      'theorem|10 def abbrev class structure instance set_option ' +
    ...
    built_in:
      ...
      'existsi|10 rcases intros introv by_cases ' +
      'focus propext ' +
      'refine eapply fapply apply_with apply_instance ' +
      'rename revert generalize specialize clear ' +
      'contradiction by_contradiction by_contra trivial exfalso ' +
      'symmetry transitivity destruct econstructor ' +
      'injection injections ' +
...
// A version of the Lean language above with special highlighting
// enabled for tactics, but rfl as a constant, due to example code in
// the last chapter that needs it.
// name may be a variable elsewhere.
hljs.registerLanguage("leantacnorfl", function(hljs) {
  var LEAN_KEYWORDS = {
    $pattern: /#?\w+/,
    keyword:
      'theorem|10 def abbrev class structure instance set_option ' +
      ...
      'import open export prelude renaming hiding ' +
      'calc match nomatch with do by let have extends ' +
      'induction simp dsimp simpa simp_arith simp_intros exact funext assumption cases case skip unfold rw|10 intro constructor split generalizing apply ' +
      'for in unless try catch finally mutual mut return continue break where rec while repeat ' +
      'syntax macro_rules macro deriving ' +
      'termination_by ' +
      'fun ' +
